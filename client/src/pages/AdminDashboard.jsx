...
const AdminDashboard = () => {
  ...
  const fetchEmergencies = async () => {
    try {
      const response = await fetch('https://roadengo.parrotconsult.com/api/emergency');
      const data = await response.json();
      setEmergencies(data);
    } catch (error) {
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch('https://roadengo.parrotconsult.com/api/admin/dashboard', {
        headers: {
  ...
  const updateAppointmentStatus = async (id, status) => {
    try {
      await fetch(`https://roadengo.parrotconsult.com/api/appointments/${id}`, {
        method: 'PATCH',
  ...
  const updateInquiryStatus = async (id, status) => {
    try {
      await fetch(`https://roadengo.parrotconsult.com/api/inquiries/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status })
      });
